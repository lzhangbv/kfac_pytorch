...
# Adam
node_rank=5
#kfac=0 use_adam=1 horovod=$horovod nworkers=$nworkers rdma=$rdma node_rank=$node_rank node_count=$node_count n_layers=$n_layers lr_mul=0.5 warmup=4000 batch_size=$batch_size epochs=$epochs bash train_multi30k.sh
kfac=0 use_adam=1 horovod=$horovod nworkers=$nworkers rdma=$rdma node_rank=$node_rank node_count=$node_count n_layers=$n_layers lr_mul=0.5 warmup=4000 batch_size=$batch_size epochs=400 bash train_multi30k.sh
kfac=0 use_adam=1 horovod=$horovod nworkers=$nworkers rdma=$rdma node_rank=$node_rank node_count=$node_count n_layers=$n_layers lr_mul=0.5 warmup=4000 batch_size=$batch_size epochs=400 bash train_multi30k.sh
